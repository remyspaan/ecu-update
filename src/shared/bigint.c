...
void bigint_mul(unsigned char *r, const unsigned char *a, const unsigned char *b, unsigned int len) 
{
  ...
  for (i=0; i<len; i++) {
    ...
    for (j=0; j<len; j++) {
      //t=r[i+j]+a[i]*b[j] + (t>>8);
      t=r[i+j]+(crypto_uint16)a[i]*b[j] + (t>>8);
      r[i+j]=(unsigned char )(t & 0xFF);
    }
    r[i+len]=(unsigned char )(t>>8);
  }
}
